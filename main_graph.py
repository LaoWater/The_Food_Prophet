...
def calculate_initial_slope(quality_index, timing_index, logistics_index):
    ...
    normalized_quality = (quality_index + 1) / 2
    normalized_timing = (timing_index + 1) / 2
    normalized_logistics = (logistics_index + 1) / 2
...
# Save plot to IO module for integration in Web Applicaiton
# Save plot to a BytesIO object
img = io.BytesIO()
plt.savefig(img, format='png')

ks2_graph = generate_plot(user_input)