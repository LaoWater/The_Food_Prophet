...
const StomachFullnessGraph = () => {
  ...
  const [isWorkerReady, setIsWorkerReady] = useState(false); // New state variable
  const svgRef = useRef();
  ...
  }, [isWorkerReady]); // Correct dependency array
  ...
  }, [isWorkerReady]); // Dependency on isWorkerReady only
